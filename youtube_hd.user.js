...
;(function () {
  ...
  function debugLog(message) {
    if (DEBUG) console.log("YTHD | " + message)
  }
  ...
  function setResolution(ytPlayer, resolutionList) {
    ...
    if (resolutionList.indexOf(res) < resolutionList.indexOf(currentQuality)) {
      ...
      if (
        !useButtons &&
        settings.flushBuffer &&
        currentQuality !== resolutionList[nextBestIndex]
      ) {
    ...
    if (useButtons) {
      ...
      if (shouldPremium) {
        resLabel = [...ytPlayer.getAvailableQualityData()].find(
          q => q.quality == res && q.qualityLabel.includes("Premium")
        ).qualityLabel
      }

      let settingsButton = doc.querySelector(
      ...
      let qualityButton = document.evaluate(
        './/*[contains(text(),"' +
          heights[resolutionList.indexOf(res)] +
    ...
    if (win.location.href.indexOf("/watch") !== -1) {
      ...
      if (pageManager && !pageManager.hasAttribute("theater")) {
        if (settings.enableErrorScreenWorkaround) {
          const styleContent =
            "#error-screen { z-index: 42 !important } .ytp-error { display: none !important }"
        ...
        try {
          ...
          pageManager.dispatchEvent(
            new CustomEvent("yt-set-theater-mode-enabled", {
              detail: { enabled: true },
              bubbles: true,
              cancelable: false,
            })
        ...
        for (let i = 0; i < 3 && !pageManager.theaterValue; ++i) {
          ...
          let theaterButton =
            theaterButton ||
            unwrapElement(doc.getElementsByClassName("ytp-size-button")[0])
          theaterButton.click()
        }
      }
    }
  ...
  function computeAndSetPlayerSize() {
    ...
    if (!settings.useCustomSize) {
      ...
      if (heightOffsetEl && mastheadContainerEl) {
        ...
        mastheadPadding =
          parseInt(win.getComputedStyle(mastheadContainerEl).paddingBottom, 10) * 2
      }

      let i = Math.max(resolutions.indexOf(settings.targetRes), 0)
      height = Math.max(
        Math.min(heights[i], win.innerHeight - (mastheadHeight + mastheadPadding)),
        270
      )
    }
...
min-height: " +
  ...
  function main() {
    ...
    if (
      settings.changePlayerSize &&
      win.location.host.indexOf("youtube.com") !== -1 &&
      win.location.host.indexOf("gaming.") === -1
    ) {

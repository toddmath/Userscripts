...
function getContainer(node) {
  ...
  ;[
    [
      '.irc_c[style*="visibility: visible;"][style*="transform: translate3d(0px, 0px, 0px);"]',
      VERSIONS.FEB18,
    ],
...
function findImageURL(container, version) {
  ...
  switch (version) {
    case VERSIONS.FEB18:
      image = container.querySelector(
        'img[src]#irc_mi, img[alt^="Image result"][src]:not([src^="https://encrypted-tbn"]).irc_mut, img[src].irc_mi'
      )
      break
    case VERSIONS.JUL19:
      var iframe = container.querySelector("iframe.irc_ifr")
      if (!iframe) return findImageURL(container, VERSIONS.FEB18)
  ...
  if (image === null || image.src === "" || image.src.startsWith("data")) {
    var thumbnail = document.querySelector(
      'img[name="' + container.dataset.itemId + '"]'
    )
    if (thumbnail === null) {
...
function addViewImageButton(container, imageURL, version) {
  ...
  switch (version) {
    ...
    case VERSIONS.JUL19:
      ...
      break
    case VERSIONS.OCT19:
      visitButton = container.querySelector(
  ...
  switch (version) {
    ...
    case VERSIONS.JUL19:
      ...
      break
    case VERSIONS.OCT19:
      viewImageButtonText = viewImageButton.querySelector(
...
function addLinks(node) {
  if (DEBUG) console.log("ViewImage: Trying to add links to node: ", node)
  ...
  if (DEBUG) console.log("ViewImage: Assuming site version: ", version)
  ...
  if (!imageURL) {
    if (DEBUG) console.log("ViewImage: Adding links failed, image was not found.")
...
function parseDataSource(array) {
  ...
  for (var i = 0; i < meta.length; i++) {
    try {
      ...
      if (DEBUG) console.log("ViewImage: Skipping image")
    }
  }
}
...
function parseDataSource1() {
  ...
  var end_index =
    start_index +
    document.documentElement.innerHTML.slice(start_index).indexOf(end_search)

  parseDataSource(
    JSON.parse(document.documentElement.innerHTML.slice(start_index, end_index))
  )
}

function parseDataSource2() {
  const start_search =
    /AF_initDataCallback\({key:\s'ds:2',\sisError:\s{2}false\s,\shash:\s'\d+',\sdata:function(){return\s/
...
try {
  if (document.documentElement.innerHTML.indexOf("key: 'ds:1'") != -1) {
    if (DEBUG) console.log("ViewImage: Attempting to parse data source 1.")
    parseDataSource1()
  } else if (document.documentElement.innerHTML.indexOf("key: 'ds:2'") != -1) {
    if (DEBUG) console.log("ViewImage: Attempting to parse data source 2.")
    parseDataSource2()
  } else {
    throw "Could not determine data source type."
  }

  if (DEBUG) console.log("ViewImage: Successfully created source images array.")
} catch (error) {
  if (DEBUG) {
...
var observer = new MutationObserver(function (mutations) {
  if (DEBUG) console.log("ViewImage: Mutations detected: ", mutations)
  ...
  for (var mutation of mutations) {
    if (mutation.addedNodes && mutation.addedNodes.length > 0) {
      for (node of mutation.addedNodes) {
        if (node.classList) {
          ...
          if (
            ["irc_mi", "irc_mut", "irc_ris", "n3VNCb", "r48jcc"].some(className =>
    ...
    if (
      ...
      mutation.target.classList.contains("n3VNCb", "r48jcc")
    ) {
...
// Start adding links
if (DEBUG) console.log("ViewImage: Initialising observer...")
...
if (DEBUG) console.log("ViewImage: Injecting CSS...")

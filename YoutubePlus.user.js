  ...
  var particle = {
    inject: function (is_userscript) {
      ...
      function getLocale(data) {
        ...
        if (data) {
          user_settings.extLang[lang.ytlang] = JSON.parse(
            document.documentElement.dataset.setlocale
          )
      ...
      function getLanguage(data) {
        ...
        if (data.target.readyState === 4 && data.target.status === 200) {
          ...
          user_settings.extLang[lang.ytlang].lastMod = new Date(
            data.target.getResponseHeader("Last-Modified")
          ).getTime()
        }
        user_settings.extLang.nextCheck = new Date().getTime() + 6048e5
        set("extLang", user_settings.extLang)
      ...
      function checkModified(data) {
        ...
        if (data.target.readyState === 4 && data.target.status === 200) {
          lang.fetching = true
          localXHR("GET", getLanguage, lang.urlBase + lang.ytlang + ".json", [
            "Accept",
      ...
      function lang(label) {
        ...
        lang.urlBase =
          "https://api.github.com/repos/ParticleCore/Particle/contents/Locale/"
        if (!user_settings.extLang) {
          set("extLang", {})
        }
        if (
          user_settings.GEN_LOCL_LANG &&
          user_settings.localLang &&
          user_settings.localLang[label]
        ) {
        ...
        if (!user_settings.GEN_LOCL_LANG && lang.ytlang && lang.ytlang !== "en_US") {
          if (
            user_settings.extLang[lang.ytlang] &&
            user_settings.extLang[lang.ytlang][label]
          ) {
            if (
              !lang.fetching &&
              user_settings.extLang.nextCheck &&
              user_settings.extLang.nextCheck <= new Date().getTime()
            ) {
              lang.fetching = true
              if (!is_userscript) {
                ...
                localXHR(
                  "HEAD",
                  checkModified,
                  lang.urlBase + lang.ytlang + ".json",
                  [
                    "If-Modified-Since",
                    new Date(
                      user_settings.extLang[lang.ytlang].lastMod
                    ).toUTCString(),
                  ]
                )
              }
      ...
      function getValues(menu) {
        ...
        if (user_settings) {
          ...
          while (i--) {
            ...
            if (
              ytp[i].type === "text" &&
              typeof user_settings[ytp[i].id] === "string"
            ) {
      ...
      function getMenu(section) {
        ...
        if (section === "MEN") {
          ...
          if (user_settings.GEN_LOCL_LANG && user_settings.localLang) {
            temp.content.querySelector(".P-implang").dataset.p =
              "ttl|GLB_LOCL_LANG&tnd|GLB_LOCL_LANG_CSTM"
          }
        } else if (section === "ABT") {
          temp.innerHTML =
            //
      ...
      function exportSettings(target) {
        var expCont
        if (
          target.classList.contains("P-impexp") ||
          target.classList.contains("P-implang")
        ) {
          ...
          document.getElementById("impexp-list").value = JSON.stringify(
            (target.classList.contains("P-impexp") && user_settings) ||
              user_settings.localLang ||
              language,
            undefined,
            2
          ...
          if (target.id === "implang-save") {
            set(
              "localLang",
              JSON.parse(document.getElementById("impexp-list").value)
            )
      ...
      function setBlackList(target) {
        if (target.id === "blacklist-edit") {
          ...
          document.getElementById("blacklist-edit-list").value = JSON.stringify(
            user_settings.blacklist,
            undefined,
            2
          )
          ...
          set(
            "blacklist",
            JSON.parse(document.getElementById("blacklist-edit-list").value)
          )
      ...
      function saveSettings(salt) {
        ...
        userSets = document
          .getElementById("P-content")
          .querySelectorAll("[id^='" + navId + "']")
        savedSets = user_settings
        i = userSets.length
        while (i--) {
          value =
            (userSets[i].checked &&
              (userSets[i].value === "on" || userSets[i].value)) ||
            (userSets[i].length && userSets[i].value) ||
            (userSets[i].getAttribute("type") === "text" && userSets[i].value)
          if (value) {
        ...
        if (!salt) {
          ...
          if (notification.childNodes.length < 1) {
            ...
            document
              .querySelector(".yt-masthead-logo-container")
              .appendChild(notification)
          }
          document.querySelector(
            ".appbar-guide-notification-text-content"
          ).textContent = lang("GLB_SVE_SETS")
      ...
      function navigateSettings(event) {
        if (event.target.classList.contains("P-save")) {
          ...
          event.target.classList.contains("P-reset") &&
          window.confirm(lang("GLB_RSET_CONF"))
        ) {
          ...
          event.target.classList.contains("P-impexp") ||
          event.target.id === "impexp-save" ||
          event.target.classList.contains("P-implang") ||
          event.target.id === "implang-save"
        ) {
          exportSettings(event.target)
        } else if (
          event.target.id === "blacklist-edit" ||
          event.target.id === "blacklist-save" ||
          event.target.id === "blacklist-close"
        ) {
          setBlackList(event.target)
        } else if (
          event.target.id === "P-container" ||
          event.target.id === "P-settings"
        ) {
          event =
            event.target.id === "P-settings" ? event.target : event.target.parentNode
          event.remove()
          ...
          event.target.parentNode.id === "P-sidebar-list"
        ) {
          saveSettings("no-notification")
          document.getElementById("P-content").remove()
          document
            .getElementById("P-container")
            .appendChild(getMenu(event.target.id))
        ...
        if (event.target.id === "P" && event.target.tagName !== "INPUT") {
          ...
          if (pWrapper) {
            ...
            document
              ...
              .insertBefore(pWrapper, document.getElementById("page-container"))
      ...
      function firstTime(event) {
        if (
          ...
          event.target.parentNode.className === "par_closewlcm"
        ) {
          set("firstTime", false)
      ...
      function settingsMenu() {
        ...
        notif_button = document.querySelector(
          "#yt-masthead-notifications, .notifications-container"
        )
        settings_button = document.querySelector(
          "#yt-masthead-user, #yt-masthead-signin"
        )
        if (settings_button) {
      ...
      function modComment(original) {
        return function (a) {
          ...
          is_live =
            ...
            window.ytplayer.config.args.livestream
          if (
            !window.location.search.match(/&?(lc|google_comment_id)=/) &&
            a.split("comments").length > 1 &&
            !is_live &&
            comments &&
            !comments.lazyload &&
            user_settings.VID_HIDE_COMS === "1" &&
            !comments.classList.contains("show")
          ) {
      ...
      function modAutoplay(original) {
        return function (_, b) {
          if (
            !b ||
            user_settings.plApl ||
            (!user_settings.plApl && b.feature && b.feature !== "autoplay")
          ) {
            original.apply(this, arguments)
          }
        }
      ...
      function modAutoplayFullscreen(original) {
        return function () {
          ...
          has_ended =
            api &&
            api.getCurrentTime &&
            Math.round(api.getCurrentTime()) >= Math.floor(api.getDuration())
          next_clicked =
            document.activeElement.classList.contains("ytp-button-next") ||
            document.activeElement.classList.contains("ytp-next-button")
          if (!user_settings.plApl && !next_clicked && has_ended) {
            next_button = document.querySelector(".ytp-next-button")
            if (
              next_button &&
              next_button.getAttribute("aria-disabled") === "true"
            ) {
      ...
      function scriptExit(event) {
        ...
        if (event && event.target) {
              ...
              while (i--) {
                ...
                if (typeof window._yt_www[key] === "function") {
                  if (String(window._yt_www[key]).split("player-added").length > 1) {
                    ...
                    String(window._yt_www[key]).split("window.spf.navigate").length >
                    1
                  ) {
                    window._yt_www[key] = modAutoplay(window._yt_www[key])
                    j++
                  } else if (
                    String(window._yt_www[key]).split('.set(""+a,b,c,"/",d').length >
                    1
                  ) {
          ...
          if (event.target.getAttribute("name") === "www/watch") {
            window.yt.www.watch.player.seekTo = modSeekTo(
              window.yt.www.watch.player.seekTo
            )
          }
        ...
        if (
          (event &&
            event.target &&
            event.target.getAttribute("name") === "player/base") ||
          (!window.html5Patched &&
            window.yt &&
            window.yt.player &&
          ...
          if (window._yt_player) {
            ...
            for (i = 0; i < temp.length; i++) {
              if (
                ...
                window._yt_player[temp[i]].prototype &&
                window._yt_player[temp[i]].prototype.hasNext
              ) {
                window._yt_player[temp[i]].prototype.hasNext = modAutoplayFullscreen(
                  window._yt_player[temp[i]].prototype.hasNext
                )
                break
              }
      ...
      function updatePos() {
        ...
        player = document.getElementById("movie_player")
        if (
          !document.documentElement.classList.contains("floater") ||
        ...
        if (event && !isFScreen && isFloater) {
          ...
          if (event.buttons === 1) {
            ...
            if (
              ...
              (!excluded || !excluded.contains(event.target))
            ) {
              ...
              bounds = checkBounds(
                player,
                event.clientX + dragFloater.oldPos.X,
                event.clientY + dragFloater.oldPos.Y
              )
      ...
      function iniFloater() {
        ...
        if (player) {
          ...
          if (!floaterUI && !isFScreen) {
            ...
            document
              .getElementById("part_floaterui_move")
              .addEventListener("mousedown", dragFloater)
            document
              .getElementById("part_floaterui_scrolltop")
      ...
      function alwaysActive(event) {
        ...
        if (
          ...
          ((event.which > 47 && event.which < 58) ||
            ...
            [27, 32, 35, 36, 37, 38, 39, 40, 66, 67, 79, 87, 187, 189].indexOf(
              event.which
            ) > -1) &&
          ["EMBED", "INPUT", "OBJECT", "TEXTAREA", "IFRAME"].indexOf(
      ...
      function playerState(event) {
        if (user_settings.fullBrs || user_settings.lightsOut) {
          document.documentElement.classList[
            (event < 5 && event > 0 && "add") || "remove"
          ](
            (user_settings.fullBrs && "part_fullbrowser") || "0",
            (user_settings.lightsOut && "part_cinema_mode") || "0"
          )
        }
        window.dispatchEvent(new Event("resize"))
      }
      ...
      function getThumb() {
        ...
        if (thumbnail_url) {
          ...
          window.ytplayer.config &&
          window.ytplayer.config.args &&
          window.ytplayer.config.args.thumbnail_url
        ) {
      ...
      function toggleFullBrowser(event) {
        ...
        advancedOptions.full_browser.classList[
          (user_settings.fullBrs && "add") || "remove"
        ]("active")
      ...
      function toggleFrames(event) {
        ...
        if (
          event &&
          ["EMBED", "INPUT", "OBJECT", "TEXTAREA"].indexOf(
            document.activeElement.tagName
          ) < 0 &&
          ...
          if ((event.keyCode === 37 || event.keyCode === 39) && event.shiftKey) {
            ...
            fps =
              ...
              ((advancedOptions.fps < 2 && 30) || advancedOptions.fps)
            if (fps && api) {
            ...
            event.type === "click" &&
            event.target.id === "framestep-button"
          ) {
            set("frame_step", !user_settings.frame_step)
            advancedOptions.frame_step.classList[
              (user_settings.frame_step && "add") || "remove"
            ]("active")
          }
        }
        if (
          advancedOptions.frame_step &&
          advancedOptions.frame_step.classList.contains("active")
        ) {
      ...
      function toggleShortcuts() {
        window.open(
          "https://github.com/ParticleCore/Particle/wiki/YouTube-shortcuts",
          "_blank"
        )
      }
      function toggleConsole(event) {
        if (event.target.id === "console-button") {
          ...
          event.target.classList[
            document.documentElement.classList.contains("player-console")
              ? "add"
              : "remove"
          ]("close")
          set(
      ...
      function togglePlay() {
        ...
        document.documentElement.classList[
          (user_settings.VID_PLR_ATPL && "add") || "remove"
        ]("part_autoplayon")
        document
          .getElementById("autoplay-button")
          .classList[(user_settings.VID_PLR_ATPL && "add") || "remove"]("active")
      }
      function toggleLoop(event) {
        ...
        if (videoPlayer) {
          ...
          if (event) {
            advancedOptions.loop_button.classList[
              (!user_settings.loopVid && "add") || "remove"
            ]("active")
          }
        }
      ...
      function toggleCinemaMode(event) {
        var plrState =
          api &&
          api.getPlayerState &&
          api.getPlayerState() < 5 &&
          api.getPlayerState() > 0
        set("lightsOut", event ? !user_settings.lightsOut : true)
        advancedOptions.cinema_mode.classList[
          (user_settings.lightsOut && "add") || "remove"
        ]("active")
        if (event && plrState) {
          document.documentElement.classList[
            (user_settings.lightsOut && "add") || "remove"
      ...
      function handleToggles(event) {
        if (
          ...
          advancedOptions.actions[event.target.dataset.action]
        ) {
      ...
      function hookButtons() {
        ...
        advancedOptions.full_browser =
          advancedOptions.controls.querySelector("#fullbrowser-button")
        advancedOptions.cinema_mode =
          advancedOptions.controls.querySelector("#cinemamode-button")
        advancedOptions.loop_button =
          advancedOptions.controls.querySelector("#loop-button")
        advancedOptions.frame_step =
          advancedOptions.controls.querySelector("#framestep-button")
        ...
        if (
          user_settings.loopVid &&
          !advancedOptions.loop_button.classList.contains("active")
        ) {
          advancedOptions.loop_button.classList.add("active")
          toggleLoop()
        }
        if (
          user_settings.fullBrs &&
          !advancedOptions.full_browser.classList.contains("active")
        ) {
          advancedOptions.full_browser.classList.add("active")
          toggleFullBrowser()
        }
        if (
          user_settings.lightsOut &&
          !advancedOptions.cinema_mode.classList.contains("active")
        ) {
          advancedOptions.cinema_mode.classList.add("active")
          toggleCinemaMode()
        }
        if (
          user_settings.frame_step &&
          !advancedOptions.frame_step.classList.contains("active")
        ) {
      ...
      function advancedOptions() {
        ...
        if (window.location.pathname === "/watch" && header && !cnslBtn) {
          ...
          if (user_settings.VID_PLR_ATPL) {
            advancedOptions.controls.content
              .querySelector("#autoplay-button")
              .classList.add("active")
          }
          advancedOptions.controls = setLocale(
            advancedOptions.controls.content
          ).firstChild
      ...
      function setButtons() {
        ...
        while (i--) {
          ...
          j = temp.length
          while (j--) {
            thumb = temp[j].querySelector(
              ".yt-lockup .yt-lockup-thumbnail, .thumb-wrapper"
            )
            if (thumb) {
              if (
                user_settings.GEN_PPOT_ON &&
                !thumb.querySelector(".popoutmode") &&
                !/channel/.test(temp[j].firstChild.className)
              ) {
                ...
                button.content.firstChild.dataset.link =
                  temp[j].querySelector("a[href*='/watch?v']").href
                thumb.appendChild(setLocale(button.content).firstChild)
              }
              if (
                user_settings.BLK_ON &&
                list[i] !== "no_channel_id" &&
                !thumb.querySelector(".blacklist") &&
                window.yt.config_.PAGE_NAME !== "channel" &&
      ...
      function delVideos() {
        ...
        while (i--) {
          temp =
            modThumbs.thumbs["channel/" + blacklist[i].ucid] ||
            modThumbs.thumbs["user/" + blacklist[i].canonical]
          if (temp) {
            j = temp.length
            while (j--) {
            ...
            if (!temp.length) {
              delete modThumbs.thumbs["channel/" + blacklist[i].ucid] ||
                modThumbs.thumbs["user/" + blacklist[i].canonical]
            }
            temp = false
          }
        ...
        temp = document.querySelectorAll(
          ".feed-item-container:not(.search-history-item)"
        )
        i = temp.length
        while (i--) {
      ...
      function modThumbs() {
        if (
          (user_settings.BLK_ON || user_settings.GEN_PPOT_ON) &&
          !window.opener &&
          window.yt &&
          window.yt.config_ &&
          /watch|index|feed|channel|results/.test(window.yt.config_.PAGE_NAME)
        ) {
          getVideos()
      ...
      function setVideoCount() {
        ...
        enhancedDetails.link.href =
          window.location.origin + "/channel/" + enhancedDetails.user + "/videos"
        enhancedDetails.username.appendChild(enhancedDetails.link)
      }
      function updateVideoCount(details) {
        details = details.target.response.querySelector(
          ".pl-header-details li:nth-child(2)"
        )
        if (details) {
      ...
      function getVideoCount() {
        ...
        if (
          !document.getElementById("uploaded-videos") &&
          enhancedDetails.username
        ) {
          ...
          if (cid[enhancedDetails.user]) {
            ...
            localXHR(
              "GET",
              updateVideoCount,
              "/playlist?list=" + enhancedDetails.user.replace("UC", "UU"),
              "doc"
            )
          }
        }
      ...
      function getChannelInfo(details) {
        ...
        if (!isLive) {
          ...
          details = details.target.response.querySelectorAll(
          ...
          if (
            details &&
            details.length > 0 &&
            enhancedDetails.watchTime.textContent.split("·").length < 2
          ) {
            enhancedDetails.watchTime.textContent +=
              " · " + details[retry ? 0 : 1].textContent
          } else if (
            retry &&
            window.ytplayer &&
            window.ytplayer.config &&
            ...
            localXHR(
              "GET",
              getChannelInfo,
              "/channel/" + window.ytplayer.config.args.ucid + "/videos",
              "doc"
            )
          }
        }
      ...
      function getPublishedTime() {
        ...
        if (
          enhancedDetails.watchTime &&
          !enhancedDetails.watchTime.fetching &&
          window.ytplayer &&
          window.ytplayer.config
        ) {
      ...
      function createButton(type, label, bool, call) {
        ...
        button.content.firstChild.classList.add(
          "yt-uix-button-icon-watch-appbar-" + type + "-video-list"
        )
        button = setLocale(button.content).firstChild
        playlistControls.plBar.className = playlistControls.plBar.className.replace(
          "radio-playlist",
          ""
        )
      ...
      class xhrPatch {
        constructor() {
          ...
          if (this.readyState === 4) {
            ...
            temp = modArgs(temp)
            temp = encodeURIComponent(
              JSON.stringify(temp.args).split(/\{"([\w\W]*?)"}/)[1]
            )
              .replace(/%5C%5C/g, "%5C")
              .replace(/%5C%22/g, "%22")
              .replace(/%22%2C%22/g, "&")
      ...
      function hideVolume() {
        if (
          volumeWheel.cBottom &&
          volumeWheel.cBottom.classList.contains("ytp-volume-slider-active")
        ) {
      ...
      function volumeWheel(event) {
        ...
        canScroll =
          event &&
          (!fsPl || (fsPl && !fsPl.contains(event.target))) &&
          (!ivCard || (ivCard && !ivCard.contains(event.target))) &&
          (!pSets || (pSets && !pSets.contains(event.target)))
        if (
          event &&
          api &&
          player &&
          canScroll &&
          (event.target.id === api || api.contains(event.target))
        ) {
          event.preventDefault()
      ...
      function dragPopOut(event) {
        ...
        if (
          ...
          !isFScreen &&
          (!excluded ||
            (event.target !== excluded && !excluded.contains(event.target)))
        ) {
          if (event.buttons === 1) {
            if (event.type === "mousedown") {
              ...
              window.moveBy(
                event.clientX - window.oldPos.X,
                event.clientY - window.oldPos.Y
              )
      ...
      function popPlayer(url) {
        ...
        pop_url =
          (!url.target && url) ||
          window.location.href.split(/&t=[0-9]+|#t=[0-9]+|&time=[0-9]+/).join("")
        if (
          url.target &&
          video &&
          video.currentTime &&
      ...
      function setSubPlaylist(event) {
        ...
        if (event.target && event.target.id === "subscription-playlist") {
          ...
          button.href =
            "/watch_videos?title=" + subPlaylist.list_title + "&video_ids=" + list
        }
      }
      function subPlaylist() {
        ...
        button = document.getElementById("subscription-playlist")
        subPlaylist.list_title = document.querySelector(
          "#subscriptions-guide-item [href='/feed/subscriptions']"
        )
        subPlaylist.video_list = document.getElementsByClassName(
          "addto-watch-later-button"
        )
        if (
          user_settings.GEN_SUB_LIST &&
          browse_items &&
          window.location.href.split("/feed/subscriptions").length > 1 &&
          !button &&
          subPlaylist.list_title &&
          subPlaylist.video_list
        ) {
          button = document.createElement("template")
          button.innerHTML =
            //
      ...
      function loadComments(event) {
        if (
          event.target &&
          event.target.parentNode &&
          event.target.parentNode.id === "P-show-comments"
        ) {
          ...
          modComments.wrapper.querySelector("button").textContent = lang(
            (modComments.comments.classList.contains("show") && "HIDE_CMTS") ||
              "SHOW_CMTS"
          )
        }
      }
        ...
        if (is_small !== "") {
          if (user_settings.VID_PLR_FIT_WDTH) {
            if (user_settings.VID_PLR_FIT_WDTH.match(/%/)) {
              max_width =
                (window.parseInt(user_settings.VID_PLR_FIT_WDTH) / 100) * temp
            } else {
      ...
      function modComments() {
        ...
        if (
          ...
          !document.getElementById("P-show-comments") &&
          user_settings.VID_HIDE_COMS === "1"
        ) {
          ...
          modComments.comments.parentNode.insertBefore(
            modComments.wrapper,
            modComments.comments
          )
      ...
      function setCustomStyles(clss) {
        document.documentElement.classList[user_settings[clss] ? "add" : "remove"](
          customStyles.custom_styles[clss]
        )
      }
      function customStyles() {
        var child,
          ...
          ads_list
        comments = document.getElementById("watch-discussion")
        ytGrid = document.querySelector(
          ".yt-uix-menu-top-level-flow-button:last-child a"
        )
        ...
        if (
          window.yt &&
          window.yt.config_ &&
          window.yt.config_.PAGE_NAME === "shared_conversation"
        ) {
          window.stop()
      ...
      function modArgs(config) {
        ...
        if (window.name === "popOut") {
          ...
          if (
            can_share &&
            can_share.dataset.shareable === "False" &&
            !config.args.video
          ) {
            config.args.video = []
            ...
            for (i = 0; i < length; i++) {
              config.args.video[i] = {
                encrypted_id: videos[i].getAttribute("data-video-id"),
              }
            }
          }
        ...
        if (config.args.caption_audio_tracks && user_settings.VID_PLR_CC) {
          config.args.caption_audio_tracks = config.args.caption_audio_tracks
            .split(/&d=[0-9]|d=[0-9]&/)
            .join("")
        }
        ...
        if (
          user_settings.VID_PLR_ADS &&
          (!user_settings.VID_SUB_ADS ||
            (user_settings.VID_SUB_ADS && !config.args.subscribed))
        ) {
      ...
      function generalChanges() {
        ...
        if (
          user_settings.GEN_YT_LOGO_LINK &&
          window.yt &&
          window.yt.config_ &&
          window.yt.config_.LOGGED_IN
        ) {
          logo = document.querySelector(
            "map[name='doodle'] > area, a#logo-container"
          )
        ...
        if (
          user_settings.VID_LAYT_AUTO_PNL &&
          window.location.pathname === "/watch" &&
          description
        ) {
      ...
      function playerMode() {
        ...
        if (user_settings.VID_PLR_SIZE_MEM) {
          ...
          if (
            ...
            pageElement.classList[user_settings.theaterMode ? "add" : "remove"](
              "watch-wide"
            )
            pageElement.className = pageElement.className.replace(
              user_settings.theaterMode ? "non-" : "watch-stage",
              user_settings.theaterMode ? "" : "watch-non-stage"
            )
            playerElement.className = user_settings.theaterMode
              ? playerElement.className.replace("small", "large")
      ...
      function addToBlacklist(data, name) {
        ...
        if (data !== "" && name !== "") {
          ...
          user_settings.blacklist.push({ name, ucid, canonical })
        }
      }
      function checkBlacklistEntry(data) {
        ...
        for (i = 0; i < user_settings.blacklist.length; i++) {
          if (
            user_settings.blacklist[i][type] !== "" &&
            value === user_settings.blacklist[i][type]
          ) {
            return true
          }
        }
      ...
      function updateBlacklistModel() {
        ...
        new_blacklist.sort(function (before, after) {
          return before.name.localeCompare(after.name)
        })
        user_settings.blacklist = new_blacklist
        set("blacklist", user_settings.blacklist)
      }
      var api, cid, language, user_settings, default_settings
      if (isMaterial()) {
        return
      }
      cid = {}
      user_settings = JSON.parse(
        document.documentElement.dataset.user_settings || null
      )
      ...
      language = {
        ...
        GLB_RSET: "Reset",
        GLB_RSET_CONF:
          "This will reset YT+ settings and the blacklist will be erased. Do you want to continue?",
        GLB_SVE: "Save",
        ...
        ABT_THKS: "Thanks to:",
        ABT_THKS_YEPPHA:
          ", who's work inspired the creation of this project, without whom none of this would exist today.",
        ABT_THKS_USERSCRIPT:
          " for making the task of developing and shipping third party software incredibly easier.",
        ABT_THKS_STACKOV:
          " for all of its priceless information which greatly contributes for software development.",
      ...
      DOMParser.prototype.parseFromString = checkDomParser(
    ...
    filterChromeKeys: function (keys) {
      if (keys[particle.id] && keys[particle.id].new_value) {
        document.documentElement.dataset.parreceive = JSON.stringify(
          (keys[particle.id].new_value &&
            keys[particle.id].new_value[particle.id]) ||
            keys[particle.id].new_value ||
            {}
        )
      }
    ...
    main: function (event) {
      ...
      if (event) {
        ...
        if (particle.is_userscript) {
          ...
          holder.href =
            "https://particlecore.github.io/Particle/stylesheets/YouTubePlus.css?v=1.9.6"
          document.documentElement.appendChild(holder)
        }
        holder = document.createElement("script")
        holder.textContent =
          "(" + particle.inject + "(" + particle.is_userscript + "))"
        document.documentElement.appendChild(holder)
        if (!particle.is_userscript) {
